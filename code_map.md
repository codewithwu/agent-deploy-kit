...
- **当前状态**：`weather_agent` 智能体（`get_weather` 工具 + `longcat` 模型） + FastAPI `/api/chat` 同步接口 + `/api/chat/stream` SSE 流式接口 + React 聊天界面（流式渲染中间步骤） + `backend/agent_loader` 按 `AGENT_NAME` 环境变量动态加载智能体 + `backend/auth` JWT/Redis 黑名单/角色守卫 8 端点 + SQLAlchemy 同步/异步双 session（`backend/db`）+ `backend/models` 用户/角色模型 + Alembic 迁移（`backend/alembic`，首个迁移 `add role and user_roles`） + `init_admin` 幂等 CLI + pytest/Vitest 烟测 + 前端鉴权 + 路由守卫（`react-router-dom` + `AuthContext` + 401 自动 refresh 单例 + `LoginPage`/`RegisterPage`/`ChatPage`/`SettingsPage`/`NotFoundPage`）
...
│   ├── auth/                    # JWT 认证 + Redis 黑名单 + 角色守卫
│   │   ├── __init__.py          # 导出 router
│   │   ├── config.py            # AuthSettings（pydantic-settings）
│   │   ├── redis_client.py      # Redis 单例 + lifespan
│   │   ├── security.py          # 哈希/JWT/黑名单
│   │   ├── schemas.py           # 认证 Pydantic 模型
│   │   ├── deps.py              # get_current_user / require_role
│   │   ├── service.py           # register/login/refresh 等业务
│   │   └── routes.py            # /api/auth/* 8 端点
│   ├── db/                      # SQLAlchemy 同步/异步双 session
│   │   ├── __init__.py          # 统一导出 Base / engine / session
│   │   ├── base.py              # DeclarativeBase
│   │   ├── session.py           # 同步 engine + get_db
│   │   └── async_session.py     # 异步 engine + get_async_db
│   ├── models/                  # SQLAlchemy 模型
│   │   ├── __init__.py          # 导出 User / UserRole
│   │   └── user.py              # User + UserRole（CheckConstraint）
│   └── alembic/                 # Alembic 迁移
│       ├── README               # 默认说明
│       ├── env.py               # 读 DATABASE_URL + 自动 import 全部 model
│       ├── script.py.mako       # 迁移文件模板
│       └── versions/            # 迁移版本（首版 6d89c8106428）
...
│       ├── components/          # Sidebar / ChatWindow / MessageList / MessageBubble / ChatInput / EmptyState / TopBar / UserMenu / LoadingScreen / ProtectedRoute
│       │   ├── auth/            # LoginForm / RegisterForm / ChangePasswordForm / DeleteAccountDialog
│       │   └── ui/              # shadcn/ui 原子组件（button / dropdown-menu / scroll-area / sheet / sonner / textarea / input / label / card / alert / avatar / dialog）
│       ├── context/             # ChatContext / AuthContext
│       ├── hooks/               # useChat / useConversations
│       ├── lib/                 # api / storage / utils / stepContent / stepDescription / tokenStorage / authEvents / apiClient / authApi
│       ├── pages/               # LoginPage / RegisterPage / ChatPage / SettingsPage / NotFoundPage
│       ├── AppRoutes.tsx        # 路由表（启动 loading + 受保护路由 + 404）
...
| `.env` | 本地环境变量（含各 LLM 提供商 API Key + `DATABASE_URL` / `DATABASE_ASYNC_URL` / `REDIS_URL`） | 新增/轮换 API Key、调换本地 LLM 地址、改 DB/Redis 连接 |
| `alembic.ini` | Alembic 配置：`script_location = backend/alembic`；`sqlalchemy.url` 由 `env.py` 覆盖 | 改迁移目录/调整 Alembic 行为 |
...
| `backend/models/__init__.py` | 导出 `User`、`UserRole`（被 alembic `env.py` 隐式 import） | 新增/删除模型时同步 |
| `backend/db/__init__.py` | 统一导出同步/异步 session 与 Base | 新增 DB 子模块时同步 |
| `backend/db/base.py` | `Base(DeclarativeBase)`：所有 SQLAlchemy 模型的基类 | 改模型基类（如加 mixin） |
| `backend/db/session.py` | 同步 `engine` / `SessionLocal` / `get_db` 依赖注入；读 `DATABASE_URL` | 改同步 session/连接池策略 |
| `backend/db/async_session.py` | 异步 `async_engine` / `AsyncSessionLocal` / `get_async_db`；读 `DATABASE_ASYNC_URL` | 改异步 session/连接池策略 |
| `backend/alembic/env.py` | Alembic 入口：从 `.env` 读 `DATABASE_URL` + `pkgutil` 自动 import `backend.models.*` 注入 `Base.metadata` | 改迁移源数据库/调整 model 扫描策略 |
| `backend/alembic/script.py.mako` | `alembic revision` 生成的迁移文件模板 | 改迁移文件标准模板 |
| `backend/alembic/versions/` | 迁移版本文件（首版 `2026_06_16_075640-6d89c8106428_add_role_and_user_roles.py`：建 `users`/`user_roles` 表 + 数据回填） | 增/改迁移时新增文件 |
...
| `frontend/src/components/` | Sidebar、ChatWindow、MessageList、MessageBubble、ChatInput、EmptyState、TopBar、UserMenu、LoadingScreen、ProtectedRoute（每个 `*.tsx` 旁有同名 `*.test.tsx`） | 增/改 UI 视图组件 |
| `frontend/src/components/auth/` | LoginForm、RegisterForm、ChangePasswordForm、DeleteAccountDialog（每个 `*.tsx` 旁有同名 `*.test.tsx`） | 增/改鉴权表单/弹窗 |
| `frontend/src/components/ui/` | shadcn/ui 原子组件（button / dropdown-menu / scroll-area / sheet / sonner / textarea / input / label / card / alert / avatar / dialog） | 升级 shadcn 组件、新增原子组件 |
| `frontend/src/context/ChatContext.tsx` | 全局聊天状态 Context | 增/改跨组件状态 |
| `frontend/src/context/AuthContext.tsx` | 全局鉴权状态（`status` / `user`）+ 5 个动作方法（`login` / `register` / `logout` / `changePassword` / `deleteAccount`）；启动时 `verify` 自动恢复登录态；订阅 `authEvents` 处理 refresh 失败 | 增/改鉴权状态机、调整启动流程 |
| `frontend/src/lib/tokenStorage.ts` | localStorage 读写 access / refresh token（keys `adk:access_token:v1` / `adk:refresh_token:v1`） | 调整 token 持久化 key 或介质 |
| `frontend/src/lib/authEvents.ts` | 进程内事件总线（当前只有 `logout`），供 `apiClient` 通知 `AuthContext` | 增/改跨模块事件 |
| `frontend/src/lib/apiClient.ts` | `apiFetch(path, opts)` + `AuthApiError`；401 + access token 时自动 `/api/auth/refresh`（单例 Promise 共享并发）后重放原请求 | 调整 401 处理/refresh 语义 |
| `frontend/src/lib/authApi.ts` | 8 个鉴权端点纯函数封装（register / login / logout / refresh / verify / me / changePassword / deleteMe） | 后端鉴权契约变更 |
| `frontend/src/pages/LoginPage.tsx` | 登录页（已登录跳 `/`，支持 `?deleted=1` 提示） | 调整登录页布局/跳转逻辑 |
| `frontend/src/pages/RegisterPage.tsx` | 注册页（已登录跳 `/`） | 调整注册页布局 |
| `frontend/src/pages/ChatPage.tsx` | 聊天主页（TopBar + Sidebar + ChatWindow） | 调整聊天主页布局 |
| `frontend/src/pages/SettingsPage.tsx` | 设置页（账户信息 + 修改密码 + 注销账户 3 段卡片） | 调整设置页内容/布局 |
| `frontend/src/pages/NotFoundPage.tsx` | 404 页（"页面不存在" + 回首页） | 调整 404 文案/操作 |
| `frontend/src/AppRoutes.tsx` | 路由表：启动 loading + `/login` `/register` 公开 + `/` `/settings` 受保护（`<Outlet/>`）+ 404 catch-all | 增/改路由表 |
...
| `2026-06-16-frontend-auth-routing-*.md` | 前端鉴权 + 路由守卫（react-router-dom + AuthContext + 401 refresh 单例 + 4 个公开/受保护页面）的设计与计划 | 历史归档 |
...
| 调整角色集合（user/admin → 别的） | `backend/models/user.py`（CheckConstraint）+ `backend/alembic/versions/` 新迁移 + `init_admin.py` |
| 新增/改 SQLAlchemy 模型 | `backend/models/<name>.py` + `backend/models/__init__.py` 导出 + `uv run alembic revision --autogenerate -m "<msg>"` |
| 跑数据库迁移 | `uv run alembic upgrade head`（迁回 `downgrade -1`） |
...
| 调整数据库连接 | `.env`（`DATABASE_URL` / `DATABASE_ASYNC_URL` / `DB_*`） |
| 调整同步/异步 session 策略 | `backend/db/session.py`（同步）/ `backend/db/async_session.py`（异步） |
| 新增/调整前端鉴权端点 | `backend/auth/routes.py` + `backend/auth/schemas.py` ↔ `frontend/src/lib/authApi.ts` + `frontend/src/context/AuthContext.tsx` |
| 调整 401 自动 refresh 行为 | `frontend/src/lib/apiClient.ts`（`refreshing` 单例 + `apiFetch` 401 分支） |
| 新增前端页面 | `frontend/src/pages/<Name>Page.tsx` + `frontend/src/AppRoutes.tsx` 加路由 + 受保护页同步挂 `ProtectedRoute` |
| 调整 token 存储 key/介质 | `frontend/src/lib/tokenStorage.ts`（access/refresh key 改版本号 `v1` → `v2` 触发用户重登） |
