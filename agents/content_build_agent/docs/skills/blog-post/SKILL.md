...
> **⚠️  这是操作手册,不是可直接调用的 tool。**
> 启用本 skill 的步骤:
> 1. 用户提出"写博客/技术文章/how-to/教程"等需求时,**先用 `read_file` 完整读取本文件**
> 2. 按下面的步骤依次调用真正的工具:
>    - `task(subagent_type="researcher", ...)` → 调研
>    - `write_file(path=".../blogs/<slug>/index.md", content=...)` → 落盘正文
>    - `generate_cover(prompt=..., slug=<slug>)` → 出封面图
> 3. 不要尝试用 `blog-post` 当 tool_name 调用,会失败
